...
import {
    KeyRound, ShieldCheck, ShieldOff, RefreshCw,
    UserX, X, ChevronDown, ChevronUp,
...
// ── Tipos internos ─────────────────────────────────────────────────────────────
interface UsuarioData { id: number; login: string; activo: boolean; }
interface IeDisponible { id: number | null; label: string; codModular: string | null; }
...
// Cambiar contraseña
const passwordData   = ref({ password: '', password_confirmation: '', error: '' });
...
async function cargarDatos() {
    ...
    try {
        ...
        modulosExpandidos.value = new Set(Object.keys(data.permisosPorModulo ?? {}));
    } catch (e: any) {
...
async function cargarPermisosIe(ieId: number | null) {
    ...
    try {
        ...
        const res = await fetch(`/api/usuarios/${usuario.value.id}/permisos-ie?ie_id=${param}`, {
            headers: { 'X-Requested-With': 'XMLHttpRequest' },
            credentials: 'same-origin',
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
...
function moduloSeleccionado(modulo: string): boolean {
    return permisosPorModulo.value[modulo]?.every(p => permisosSeleccionados.value.has(p.id)) ?? false;
}
...
async function guardarPermisos() {
    ...
    try {
        const res = await fetch(`/api/usuarios/${usuario.value.id}/permisos-ie`, {
            method: 'POST',
            headers: {
...
async function submitPassword() {
    ...
    if (passwordData.value.password.length < 8) { passwordData.value.error = 'Mínimo 8 caracteres'; return; }
    if (passwordData.value.password !== passwordData.value.password_confirmation) {
        passwordData.value.error = 'Las contraseñas no coinciden'; return;
    }
    savingPassword.value = true;
    try {
        const res = await fetch(`/usuarios/${usuario.value.id}/cambiar-password`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-Requested-With': 'XMLHttpRequest', 'X-CSRF-TOKEN': getCsrf() },
            credentials: 'same-origin',
            body: JSON.stringify({ password: passwordData.value.password, password_confirmation: passwordData.value.password_confirmation }),
        });
        if (!res.ok) {
            const d = await res.json().catch(() => ({}));
            passwordData.value.error = d?.errors?.password?.[0] ?? 'Error al cambiar contraseña';
        } else {
            passwordData.value = { password: '', password_confirmation: '', error: '' };
            section.value = 'main';
        }
...
<template>
    <Teleport to="body">
        <div
            ...
            @click.self="close"
        >
            <div class="bg-background rounded-lg shadow-xl w-full mx-4 flex flex-col max-h-[90vh]"
                 :class="section === 'permisos' ? 'max-w-2xl' : 'max-w-lg'">

                <!-- Cabecera -->
                <div class="flex items-start justify-between border-b px-5 py-4 shrink-0">
                    <div>
                        <h2 class="text-base font-semibold leading-tight">Usuario del Sistema</h2>
                        <p class="text-sm text-muted-foreground mt-0.5 truncate max-w-xs">{{ trabajadorNombre }}</p>
                    </div>
                    <button @click="close" class="rounded p-1 hover:bg-muted text-muted-foreground ml-4 shrink-0">
                        <X class="h-4 w-4" />
                    </button>
                </div>
                ...
                <div class="overflow-y-auto flex-1 px-5 py-4">
                    ...
                    <!-- Cargando -->
                    <div v-if="loading" class="flex items-center justify-center py-12 text-muted-foreground gap-2">
                        <RefreshCw class="h-4 w-4 animate-spin" /> Cargando...
                    </div>

                    <!-- Error -->
                    <div v-else-if="error" class="rounded border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">
                        {{ error }}
                    </div>

                    <!-- Sin usuario -->
                    <div v-else-if="!usuario" class="flex flex-col items-center gap-2 py-10 text-muted-foreground">
                        <UserX class="h-8 w-8" />
                        <p class="text-sm">Este trabajador no tiene usuario en el sistema.</p>
                    </div>
                    ...
                    <template v-else-if="section === 'main'">
                        ...
                        <div class="mb-5 flex items-center justify-between rounded-md border bg-muted/30 px-4 py-3">
                            <div>
                                <p class="text-xs text-muted-foreground">Login</p>
                                <p class="font-mono font-semibold">{{ usuario.login }}</p>
                            </div>
                            <div class="flex items-center gap-2 flex-wrap justify-end">
                                <StatusBadge :active="usuario.activo" />
                                <Button variant="outline" size="sm" @click="showToggleConfirm = true">
                                    <ShieldOff v-if="usuario.activo" class="h-3.5 w-3.5 mr-1" />
                                    <ShieldCheck v-else class="h-3.5 w-3.5 mr-1" />
                                    {{ usuario.activo ? 'Desactivar' : 'Activar' }}
                                </Button>
                                <Button variant="outline" size="sm" @click="section = 'password'">
                                    <KeyRound class="h-3.5 w-3.5 mr-1" />
                                    Contraseña
                                </Button>
                            </div>
                        </div>
                        ...
                        <div>
                            <h3 class="text-sm font-semibold mb-2">Permisos por Institución</h3>
                            <p class="text-xs text-muted-foreground mb-3">
                                Selecciona una institución para ver y editar sus permisos directos.
                                Solo aparecen las IEs donde el trabajador tiene alta activa.
                            </p>

                            <div v-if="iesDisponibles.length === 0" class="rounded border border-dashed p-4 text-center text-sm text-muted-foreground">
                                Sin instituciones disponibles (el trabajador no tiene altas activas).
                            </div>
                            ...
                            <div v-else class="grid gap-2">
                                <button
                                    ...
                                    :key="String(ie.id)"
                                    @click="section = 'permisos'; seleccionarIe(ie.id)"
                                    ...
                                    :class="ieSeleccionada === ie.id && section === 'permisos' ? 'border-primary bg-primary/5' : ''"
                                >
                                    <div>
                                        <span class="font-medium">{{ ie.label }}</span>
                                        <span v-if="ie.codModular" class="ml-2 text-xs text-muted-foreground font-mono">({{ ie.codModular }})</span>
                                    </div>
                                    <span class="text-xs text-muted-foreground">Gestionar →</span>
                                </button>
                            </div>
                        </div>
                    </template>
                    ...
                    <template v-else-if="section === 'password'">
                        <div class="space-y-4">
                            <div class="grid gap-2">
                                ...
                                <Input id="gum-pwd" type="password" v-model="passwordData.password"
                                    placeholder="Mínimo 8 caracteres" autocomplete="new-password" />
                            </div>
                            <div class="grid gap-2">
                                <Label for="gum-pwd2">Confirmar contraseña *</Label>
                                <Input id="gum-pwd2" type="password" v-model="passwordData.password_confirmation"
                                    placeholder="Repetir contraseña" autocomplete="new-password" />
                            </div>
                            <p v-if="passwordData.error" class="text-sm text-destructive">{{ passwordData.error }}</p>
                        </div>
                    </template>
                    ...
                    <template v-else-if="section === 'permisos'">
                        ...
                        <!-- IE seleccionada -->
                        <div class="mb-4 flex items-center gap-2 rounded-md border bg-muted/30 px-3 py-2">
                            <span class="text-xs text-muted-foreground">IE:</span>
                            <span class="text-sm font-semibold">{{ ieLabel }}</span>
                            <span v-if="permisosSaved" class="ml-auto text-xs text-green-600 font-medium">✓ Guardado</span>
                        </div>

                        <!-- Plantilla desde perfil -->
                        <div class="mb-4 flex items-center gap-2">
                            <span class="text-xs text-muted-foreground shrink-0">Cargar desde perfil:</span>
                            <select
                                v-model="perfilPlantillaId"
                                class="flex h-8 flex-1 rounded-md border border-input bg-background px-2 py-1 text-xs shadow-sm focus:outline-none focus:ring-1 focus:ring-ring"
                            >
                        ...
                        <!-- Cargando permisos -->
                        <div v-if="permisosCargando" class="flex items-center gap-2 py-6 text-muted-foreground justify-center">
                            <RefreshCw class="h-4 w-4 animate-spin" /> Cargando permisos...
                        </div>

                        <!-- Lista de permisos por módulo -->
                        <div v-else class="space-y-2">
                            <div v-if="modulos.length === 0" class="py-8 text-center text-sm text-muted-foreground">
                                No hay permisos configurados en el sistema.
                            </div>
                            <div v-for="modulo in modulos" :key="modulo" class="rounded-md border overflow-hidden">
                                <!-- Cabecera módulo -->
                                <div class="flex items-center gap-2 bg-muted/40 px-3 py-2">
                                    ...
                                    />
                                    <label :for="`mod-${modulo}`" class="flex-1 font-semibold capitalize text-sm cursor-pointer select-none">
                                        {{ modulo }}
                                    </label>
                                    <button @click="toggleModuloExpand(modulo)" class="text-muted-foreground hover:text-foreground">
                                        <ChevronUp v-if="modulosExpandidos.has(modulo)" class="h-4 w-4" />
                                        <ChevronDown v-else class="h-4 w-4" />
                                    </button>
                                </div>
                                <!-- Permisos del módulo -->
                                <div v-if="modulosExpandidos.has(modulo)" class="grid grid-cols-1 sm:grid-cols-2 gap-1 p-2">
                                    <label
                                        v-for="permiso in permisosPorModulo[modulo]"
                                        :key="permiso.id"
                                        class="flex items-start gap-2 cursor-pointer rounded p-1.5 hover:bg-muted/50"
                                    >
                                        <input
                                            type="checkbox"
                                        ...
                                        />
                                        <div>
                                            <p class="text-xs font-medium leading-none">{{ permiso.codigo }}</p>
                                            <p v-if="permiso.descripcion" class="text-[11px] text-muted-foreground mt-0.5">{{ permiso.descripcion }}</p>
                                        </div>
                                    </label>
                                </div>
                            </div>
                ...
                <!-- Footer -->
                <div v-if="!loading && !error && usuario" class="flex justify-between border-t px-5 py-3 shrink-0">
                    <Button
                        v-if="section !== 'main'"
                        variant="ghost" size="sm"
                        @click="section = 'main'; ieSeleccionada = undefined"
                    >
                        ← Volver
                    </Button>
                    <div v-else />
                    ...
                    <div class="flex gap-2">
                        ...
                        <Button v-if="section === 'password'" @click="submitPassword" :disabled="savingPassword">
                            {{ savingPassword ? 'Guardando...' : 'Guardar Contraseña' }}
                        </Button>
                        <Button v-if="section === 'permisos'" @click="guardarPermisos" :disabled="savingPermisos || permisosCargando">
                ...
                <!-- Footer sin usuario -->
                <div v-else-if="!loading && !error && !usuario" class="flex justify-end border-t px-5 py-3 shrink-0">
                    <Button variant="outline" @click="close">Cerrar</Button>
                </div>
            </div>
        </div>
